...
select
  ...
  events.event_id,
  row_number() over (partition by case when map.user_id is not null then map.user_id else events.domain_userid end order by events.collector_tstamp ) as user_event_index,
  row_number() over (partition by case when map.user_id is not null then map.user_id else events.domain_userid end,events.domain_sessionid order by events.collector_tstamp ) as session_event_index,
