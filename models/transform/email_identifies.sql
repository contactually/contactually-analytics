...
with events as (
  ...
  select se_label as email, domain_userid, collector_tstamp
  from events
  ...
  select se_property as email, domain_userid, collector_tstamp
  from events
  ...
  select elements.value, events.domain_userid, events.collector_tstamp
  from submits
