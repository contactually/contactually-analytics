with sessions as (
    ...
    select
        s.session_id,
        case when s.refr_urlhost_clean = 'contactually.com' then 'internal' else
    ...
    left outer join {{ ref('campaign_referers') }} c5 on (s.refr_urlhost = c5.refr_urlhost or s.refr_urlhost_clean = replace(c5.refr_urlhost, 'wwww.', ''))
    where
